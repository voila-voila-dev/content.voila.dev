...
//
// This copy wires the `/` slash menu and image upload. `@`-mentions are left
// off: they need a runtime source of people to mention, and a cinema programme
// note has no one to address.
...
function RichTextValueInput({
  ...
  // `slash` wires the `/` command menu (gated to the field's blocks); `media`
  // enables image upload + drop/paste when the field allows `image`.
  const { plugins, components } = useMemo(
    () => derivePlugins(elements, marks, { slash: true, media: MEDIA }),
    [elements, marks],
  );
