...
describe("parseMigrationFile", () => {
  it("parses a well-formed migration filename", () => {
    expect(parseMigrationFile("0007_add_posts.sql")).toEqual({
      id: 7,
      name: "add_posts",
      file: "0007_add_posts.sql",
    });
  });

  it("rejects non-migration filenames", () => {
    expect(parseMigrationFile("notes.sql")).toBeNull();
    expect(parseMigrationFile("0001_init.ts")).toBeNull();
